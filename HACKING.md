...
sources files if `run-checks` complains about the format.

You can run individual test for a sub-package by changing into that directory
and:
...
This is because we need at least 4.4+ kernel for snapd to run on Trusty, which
is available through the `linux-generic-lts-xenial` package.

If you are running Ubuntu 16.04 LTS (Xenial Xerus), use 
...
Finally, you can run the spread tests for Ubuntu 16.04 64-bit with:
