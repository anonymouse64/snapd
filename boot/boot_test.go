...
func (s *bootSetSuite) TestHappyMarkBootSuccessfulKernelUpgradeSetBootVarsPanics(c *C) {
	...
	// we will panic during the next SetBootVars
	...
	// the next kernel we boot from should be kernel1, because we are still in
	// "trying"
	c.Assert(nextBootingKernel, Equals, kernel1)

	// we also should have had kernel_status reset to ""
	...
	// trying == true, because the symlink is still there, the devicestate mgr
	...
	// TODO:UC20: move this test there instead?
