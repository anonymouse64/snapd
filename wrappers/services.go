...
func StartServices(apps []*snap.AppInfo, inter interacter) (err error) {
	...
	for _, app := range apps {
		...
		// because the service is socket-activated and so we base the decision on whether
		// to start the service/sockets off whether the sockets are all disabled or not
		...
		case len(app.Sockets) == 0 && isServiceEnabled:
			...
			// also note I don't know what to do if a service has both a socket + timer
			// but what currently happens is that the timer won't get started, which is probably reasonable
			if app.Timer != nil {
				timerService := filepath.Base(app.Timer.File())
				// enable the timer
