...
`
	...
	TestDeveloper1AccountKey = `type: account-key
authority-id: testrootorg
...
func init() {
	...
	dev1AccKey, err := asserts.Decode([]byte(TestDeveloper1AccountKey))
	if err != nil {
		panic(fmt.Sprintf("cannot decode test dev1 account assertion: %v", err))
	}

	TestRootAccount = acct
	TestRootAccountKey = accKey
	TestStoreAccountKey = storeAccKey
	Trusted = []asserts.Assertion{TestRootAccount, TestRootAccountKey, dev1AccKey}
}
