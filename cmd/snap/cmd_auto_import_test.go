...
func (s *SnapSuite) TestAutoImportIntoSpool(c *C) {
	...
	mockMountInfoFmt := `24 0 8:18 / %s rw,relatime shared:1 - squashfs /dev/sc1 rw,errors=remount-ro,data=ordered`
	restore = osutil.MockMountInfo(mockMountInfoFmt)
	defer restore()
...
func (s *SnapSuite) TestAutoImportUC20CandidatesIgnoresSystemPartitions(c *C) {
	...
	mockMountInfoFmtWithLoop := `24 0 8:18 / %[1]s%[2]s rw,relatime foo ext3 /dev/meep2 no,separator
